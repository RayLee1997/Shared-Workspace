...
**Database ID**: `30170xxx-c66e-xxx-93b3-xxxxxxf`  
**Database URL**: `https://www.notion.so/3017xxxb5c9093f`

**📌 重要规则**:

- ✅ 创建新页面时，必须指定 `parent: { page_id: "30170xxx-c66e-xxx-93b3-xxxxxxf" }`
...
mcp_notion_notion-create-pages({
  parent: {
    page_id: "30170xxx-c66e-xxx-93b3-xxxxxxf"  // ⚠️ 必须指定 Obsidian 数据库
  },
  pages: [{
    properties: {
      title: page_title,
...
2. **创建**：`mcp_notion_notion-create-pages`
   - **必需参数**：`parent.page_id = "30170xxx-c66e-xxx-93b3-xxxxxxf"`
   - 参数：`pages` (数组)
   - 限制：content < 20K 字符
